...
class Service_category extends CI_Controller{
  ...
  function get_service_catgeory_json()
  {
    ...
    $get_service_catgeory = $this->M_service_category->get_service_category($filter_value, 'ServiceCategoryName DESC');
    //print_r($get_support->row());exit();
    $baris = $get_service_catgeory->result();
    $data = array();
    foreach ($baris as $bar) {
      ...
      "IsActive" => $is_active,
      "EditButton" => "<a class='btn btn-info' href=''>Edit
    ...
    $output = array(
      "draw" => 0,
      "recordsTotal" => $get_service_catgeory->num_rows(),
      "recordsFiltered" => $get_service_catgeory->num_rows(),
      "data" => $data
    );
